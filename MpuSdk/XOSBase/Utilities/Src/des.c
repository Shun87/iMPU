...
static void spinit(DES_HANDLE Handle)
{
	char pbox[32];
	int p,i,s,j,rowcol;
	int val;
...
#ifdef PERMUTATION
	perminit(Handle->iperm,(char *)ip);
	perminit(Handle->fperm,(char *)fp);
#endif	
	...
	for (i=0; i<16; i++) {		/* key chunk for each iteration */
		...
		for (j=0; j<48; j++){	/* select bits individually */
			...
			if (pcr[pc2[j]-1]){
				...
				l = dv;
				Handle->kn[i][dr] |= bytebit[l] >> 2;
				
//				l= j % 6;
//				Handle->kn[i][j/6] |= bytebit[l] >> 2;
			}
		}
	}
}
...
static int f(DES_HANDLE Handle,unsigned int r,unsigned char subkey[8])
//unsigned int r;		/* 32 bits */
//unsigned char subkey[8];	/* 48-bit key for this round */
{
	...
	rval = 0;
	rval |= Handle->sp_[0][((rt >> 26) ^ *subkey++) & 0x3f];
	rval |= Handle->sp_[1][((rt >> 22) ^ *subkey++) & 0x3f];
	rval |= Handle->sp_[2][((rt >> 18) ^ *subkey++) & 0x3f];
	rval |= Handle->sp_[3][((rt >> 14) ^ *subkey++) & 0x3f];
	rval |= Handle->sp_[4][((rt >> 10) ^ *subkey++) & 0x3f];
	rval |= Handle->sp_[5][((rt >> 6) ^ *subkey++) & 0x3f];
	rval |= Handle->sp_[6][((rt >> 2) ^ *subkey++) & 0x3f];
	rt = (r << 1) | ((r & 0x80000000) ? 1 : 0);
	rval |= Handle->sp_[7][(rt ^ *subkey) & 0x3f];
	return rval;
}

/* Do one DES cipher round */
static void des_round(DES_HANDLE Handle,int num,unsigned int *block)
...
//unsigned int *block;
{
	 ...
	 */
	if(num & 1){
		block[1] ^= f(Handle,block[0],Handle->kn[num]);
	} else {
		block[0] ^= f(Handle,block[1],Handle->kn[num]);
	}
}          
...
void Des_Enc(DES_HANDLE Handle,char *Block)
{
	...
	if (Handle->desmode == 0){
		permute(Block,Handle->iperm,(char *)work);	/* Initial Permutation */
	}
	if ((Handle->desmode == 1) || (Handle->desmode == 2)){
		permute(Block,0,(char *)work);	/* Initial Permutation */
	}
#ifdef LITTLE_ENDIAN
...
#endif
	if (Handle->desmode == 0){
		permute((char *)work,Handle->fperm,Block);	/* Inverse initial permutation */
	}
	if ((Handle->desmode == 1) || (Handle->desmode == 2)){
		permute((char *)work,0,Block);	/* Inverse initial permutation */
	}
}
