...
int gbk2utf8(unsigned char *pgbk,unsigned char *putf8)
{
	int us_length,utf8_length;
	unsigned short *pus = NULL;
	us_length =	gb2uni(pgbk, NULL, 0);
	pus = (unsigned short *)malloc((us_length + 1)* sizeof(unsigned short));
	if (pus == NULL)
	{
		return 0;
	}
	gb2uni(pgbk, pus, us_length);
	pus[us_length] = 0;
	
	// ?aê???Unicode×a??μ?UTF8
	utf8_length = ucs2ToUtf8(pus, NULL, 0);
	ucs2ToUtf8(pus, putf8, utf8_length);
	putf8[utf8_length] = '\0';
	free(pus);
	return utf8_length;
}

int utf82gbk(unsigned char *putf8,unsigned char *pgbk)
{
	int us_length,gbk_length;
//	unsigned short us[128];
	unsigned short *pus = NULL;
	us_length = utf8ToUcs2(putf8, NULL, 0);
	pus = (unsigned short *)malloc((us_length + 1)* sizeof(unsigned short));
	if (pus == NULL)
	{
		return 0;
	}
	utf8ToUcs2(putf8, pus, us_length);
	pus[us_length] = 0;
	
	// ?ù??Unicode×a??μ?GBK
	gbk_length = uni2gb(pus, NULL, 0);
	uni2gb(pus, pgbk, gbk_length);
	pgbk[gbk_length] = '\0';
	free(pus);
	return gbk_length;
}
