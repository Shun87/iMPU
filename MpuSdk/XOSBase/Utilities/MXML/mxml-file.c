...
mxml_add_char(int  ch,			/* I  - Character to add */
	      ...
	      int  encoding)		/* I  - Character encoding */
{
  ...
  */
  ...
  while ((ch = (*getc_cb)(p, &encoding)) != EOF)
  {
	...
	*bufptr = '\0';
	...
	//add by xmwang, parse encoding
	pszDup = strdup(buffer);
	if (pszDup == NULL)
	{
		mxml_error("Before add processing instruction node _mxml_strdup failed!");
		goto error;
	}
	psz = strtok(pszDup, " =\"");
	while (psz != NULL)
	{
		if (strcmp(psz, "encoding") == 0)
		{
			psz = strtok(NULL, " =\"");
			if (psz == NULL)
			{
			...
			if (strcmp(mystrlwr(psz), "gb2312") == 0) 
			{
